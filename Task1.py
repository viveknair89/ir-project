...
def execute_system(p_datafilesdir,
    ...
    if p_createindex:
        ...
        lindexer.setOutputDirectory(TASK1_CONST)
        lindexer.startIndexing(False, False)
        lindexer.printAll()
        t2 = time.time()
    ...
    lindexfilename = ldirpathfortask + "/" + FILE_FOR_INDEX + FILE_EXT
    ldocfreqtableforunigramfilename = ldirpathfortask + "/" + FILE_FOR_DOC_FREQ_TABLE + CONSTS_FOR_UNIGRAM + FILE_EXT
    lwordcountsbyfilefilename = ldirpathfortask + "/" + FILE_FOR_WORD_COUNTS_BY_FILE_FOR + FILE_FOR_INDEX + FILE_EXT

    lrm.setNoOfDocuments(NoOfDocuments)
    lrm.setOutputDirectory(TASK1_CONST)
    ...
    endTime = time.time()
    print "Task 1 execution completed in " + str(endTime - startTime)
