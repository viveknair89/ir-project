...
def convert_list_to_dict(p_list):
...
def convert_data_from_file_to_dict_with_index(p_filename):
...
def get_sorted_dict(p_dict, p_sortbyvalue=False, p_reversed=False):

    if p_sortbyvalue:
        if p_reversed:
            p_dict = OrderedDict(reversed(sorted(p_dict.items(), key=itemgetter(1))))
        else:
            p_dict = OrderedDict(sorted(p_dict.items(), key=itemgetter(1)))
    else:
        if p_reversed:
            p_dict = OrderedDict(reversed(sorted(p_dict.items())))
        else:
