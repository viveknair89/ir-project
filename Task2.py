...
def execute_system(p_datafilesdir,
    ...
    ldirpathfortask = DIR_FOR_OUTPUT_FILES + "/" + TASK2_CONST
    ...
    if p_createindex:
        ...
        lindexer.setOutputDirectory(TASK2_CONST)
    ...
    lrm.setOutputDirectory(TASK2_CONST)
    lrm.setCanUseRelevanceInfo(False)
    ...
    lrm.processQueriesWithQueryExpansion(lquerydict, True)
    t2 = time.time()
    # print "Time for Retrieval Module: " + str(t2-t1)

    endTime = time.time()
    print "Task 2 execution completed in " + str(endTime - startTime)
